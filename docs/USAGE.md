...
VALENTINE RF automatically detects known trackers:
- Apple AirTag
- Tile
- Samsung SmartTag
- Chipolo
...
3. **Set Location** - Choose location source:
   ...
   - **Shared Location** - By default, the observer location is shared across modules
     (disable with `VALENTINE_SHARED_OBSERVER_LOCATION=false`)
...
> set `VALENTINE_ADSB_AUTO_START=true`.
...
|----------|---------|-------------|
| `VALENTINE_ADSB_HISTORY_ENABLED` | `false` | Enables history storage and reporting |
| `VALENTINE_ADSB_DB_HOST` | `localhost` | Postgres host (use `adsb_db` in Docker) |
| `VALENTINE_ADSB_DB_PORT` | `5432` | Postgres port |
| `VALENTINE_ADSB_DB_NAME` | `valentine-rf_adsb` | Database name |
| `VALENTINE_ADSB_DB_USER` | `valentine-rf` | Database user |
| `VALENTINE_ADSB_DB_PASSWORD` | `valentine-rf` | Database password |
...
1. **Install VALENTINE RF** on the remote machine
2. **Create config file** (`valentine-rf_agent.cfg`):
   ```ini
   [agent]
   name = sensor-node-1
   port = 8020
...
3. **Start the agent**:
   ```bash
   python valentine-rf_agent.py --config valentine-rf_agent.cfg
   ```

### Registering Agents in the Controller

1. Navigate to `/controller/manage` in the main VALENTINE RF instance
...
VALENTINE RF can be configured via environment variables:

| Variable | Default | Description |
|----------|---------|-------------|
| `VALENTINE_HOST` | `0.0.0.0` | Server bind address |
| `VALENTINE_PORT` | `5050` | Server port |
| `VALENTINE_DEBUG` | `false` | Enable debug mode |
| `VALENTINE_LOG_LEVEL` | `WARNING` | Log level (DEBUG, INFO, WARNING, ERROR) |
| `VALENTINE_DEFAULT_GAIN` | `40` | Default RTL-SDR gain |

Example: `VALENTINE_PORT=8080 sudo -E venv/bin/python valentine-rf.py`

## Command-line Options

```
python3 valentine-rf.py --help
