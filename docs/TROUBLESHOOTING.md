...
sudo venv/bin/python valentine-rf.py
```
...
This error occurs on Python 3.7 or 3.8. **VALENTINE RF requires Python 3.9 or later.**
...
2. In Valentine RF's ADS-B dashboard:
   - Check the **"Remote"** checkbox
   - Enter Host: `localhost` and Port: `30003`
   - Click **START**

3. Valentine RF will connect to readsb's SBS output on port 30003
...
The Listening Post uses Icecast for low-latency audio streaming (2-10 second latency). Valentine RF will automatically start Icecast when you begin listening, but you must install and configure it first.
...
**Configure Valentine RF (optional):**
...
- **"Authentication failed"**: Check the source password matches between Icecast config and Valentine RF
...
Valentine RF now logs detailed error output. Look for lines starting with `[AUDIO]`:
```
...
Run VALENTINE RF with sudo:
```bash
sudo -E venv/bin/python valentine-rf.py
```
