# Valentine RF Distributed Agent System

This document describes the distributed agent architecture that allows multiple remote sensor nodes to feed data into a central Valentine RF controller.

## Overview

The agent system uses a hub-and-spoke architecture where:
- **Controller**: The main Valentine RF instance that aggregates data from multiple agents
- **Agents**: Lightweight sensor nodes running on remote devices with SDR hardware

```
                    ┌─────────────────────────────────┐
                    │      VALENTINE RF CONTROLLER       │
                    │         (port 5050)             │
                    │                                 │
...
The controller is the main Valentine RF application:
...
### Agent Config (`valentine-rf_agent.cfg`)
...
| File | Description |
|------|-------------|
| `valentine-rf_agent.py` | Standalone agent server |
| `valentine-rf_agent.cfg` | Agent configuration template |
