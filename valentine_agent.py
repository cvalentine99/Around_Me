#!/usr/bin/env python3
"""
VALENTINE RF Agent - Remote node for distributed signal intelligence.

This agent runs on remote nodes and exposes Valentine RF's capabilities via REST API.
It can push data to a central controller or respond to pull requests.

Usage:
    python valentine_agent.py [--port 8020] [--config valentine_agent.cfg]
"""
...
# Import dependency checking from Valentine RF utils
try:
...
# =============================================================================
# Mode Manager - Uses Valentine RF's existing utilities and tools
# =============================================================================

class ModeManager:
    """
    Manages mode state using Valentine RF's existing infrastructure.

    This assumes Valentine RF (or its utilities) is installed on the agent host.
    ...
    def __init__(self):
        ...
        # Lazy-loaded Valentine RF utilities
        self._sdr_factory = None
        self._dependencies = None

    def _get_sdr_factory(self):
        """Lazy-load SDRFactory from Valentine RF's utils."""
    ...
    def _get_dependencies(self):
        """Lazy-load dependencies module from Valentine RF's utils."""
    ...
    def _check_tool(self, tool_name: str) -> bool:
        """Check if a tool is available using Valentine RF's dependency checker."""
    ...
    def _get_tool_path(self, tool_name: str) -> str | None:
        """Get tool path using Valentine RF's dependency module."""
        deps = self._get_dependencies()
    ...
    def detect_capabilities(self) -> dict:
        """Detect available tools and hardware using Valentine RF's utilities."""
        ...
        # Detect interfaces using Valentine RF's TSCM device detection
        self._detect_interfaces(capabilities)

        # Use Valentine RF's comprehensive dependency checking if available
        ...
        # Use Valentine RF's SDR detection
    ...
    # -------------------------------------------------------------------------
    # SENSOR MODE (rtl_433) - Uses Valentine RF's SDR abstraction
    # -------------------------------------------------------------------------

    def _start_sensor(self, params: dict) -> dict:
        """Start rtl_433 sensor mode using Valentine RF's SDR utilities."""
        ...
        # Try to use Valentine RF's SDR abstraction layer
        sdr_factory = self._get_sdr_factory()
        if sdr_factory:
            try:
    ...
    # -------------------------------------------------------------------------
    # ADS-B MODE (dump1090) - Uses Valentine RF's SDR abstraction
    # -------------------------------------------------------------------------

    def _start_adsb(self, params: dict) -> dict:
        """Start dump1090 ADS-B mode using Valentine RF's utilities."""
        ...
        # Try using Valentine RF's SDR abstraction for building the command
        sdr_factory = self._get_sdr_factory()
        cmd = None
    ...
    def _find_dump1090(self) -> str | None:
        """Find dump1090 binary using Valentine RF's dependency module or fallback."""
        # Try Valentine RF's tool path finder first
    ...
    # -------------------------------------------------------------------------
    # WIFI MODE (airodump-ng) - Uses Valentine RF's utilities
    # -------------------------------------------------------------------------

    def _start_wifi(self, params: dict) -> dict:
        """Start WiFi scanning using Valentine RF's UnifiedWiFiScanner."""
        interface = params.get('interface')
        ...
        # Deep scan - use Valentine RF's UnifiedWiFiScanner
        try:
    ...
    def _start_wifi_fallback(
        ...
        csv_path = '/tmp/valentine_agent_wifi'
    ...
    def _parse_airodump_csv(self, csv_path: str, gps_data: dict | None = None) -> tuple[dict, dict]:
        """Parse airodump-ng CSV file using Valentine RF's existing parser."""
        networks = {}
        clients = {}

        try:
            # Use Valentine RF's robust airodump parser (handles edge cases, proper CSV parsing)
            ...
            logger.warning("Valentine RF WiFi parser not available, using fallback")
    ...
    def _start_bluetooth(self, params: dict) -> dict:
        """Start Bluetooth scanning using Valentine RF's BluetoothScanner."""
        adapter = params.get('adapter', 'hci0')
        mode_param = params.get('mode', 'auto')
        duration = params.get('duration')

        try:
            # Use Valentine RF's BluetoothScanner
    ...
    def _parse_pager_message(self, line: str) -> dict | None:
        """Parse multimon-ng output line for POCSAG/FLEX using Valentine RF's parser."""
        try:
            # Use Valentine RF's existing pager parser
    ...
    def _start_aprs(self, params: dict) -> dict:
        ...
        if direwolf_path:
            dw_config = '/tmp/valentine_direwolf.conf'
            try:
    ...
    def _start_dsc(self, params: dict) -> dict:
        """Start DSC (VHF Channel 70) decoding using Valentine RF's DSCDecoder."""
        device = params.get('device', '0')
        gain = params.get('gain', '40')
        ppm = params.get('ppm', '0')
        ...
        try:
            ...
            # Start output reader thread using Valentine RF's DSCDecoder
            thread = threading.Thread(
    ...
    def _dsc_output_reader(self, proc: subprocess.Popen):
        """Read rtl_fm audio and decode DSC using Valentine RF's DSCDecoder."""
        mode = 'dsc'
        stop_event = self.stop_events.get(mode)

        try:
            # Use Valentine RF's DSC decoder
            from utils.dsc.decoder import DSCDecoder
            decoder = DSCDecoder(sample_rate=48000)
            logger.info("Using Valentine RF's DSCDecoder")
    ...
    def _start_tscm(self, params: dict) -> dict:
        """Start TSCM scanning - uses existing Valentine RF scanning functions."""
        # Initialize state
        ...
        # Start the combined TSCM scanner thread using existing Valentine RF functions
        thread = threading.Thread(
    ...
    def _tscm_scanner_thread(self, scan_wifi: bool, scan_bt: bool, scan_rf: bool,
        ...
        """Combined TSCM scanner using existing Valentine RF functions.
        ...
        # Import existing Valentine RF TSCM functions
        ...
        while not (stop_event and stop_event.is_set()):
            try:
                ...
                # WiFi scan using Valentine RF's function (same as local mode)
                if scan_wifi:
                    try:
                ...
                # Bluetooth scan using Valentine RF's function (same as local mode)
                if scan_bt:
                    try:
                ...
                # RF scan using Valentine RF's function (less frequently)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='VALENTINE RF Agent - Remote signal intelligence node'
    )
    parser.add_argument(
        '--port', '-p',
        type=int,
    ...
    parser.add_argument(
        '--config', '-c',
        default='valentine_agent.cfg',
        help='Configuration file (default: valentine_agent.cfg)'
    )
    parser.add_argument(
        '--name', '-n',
