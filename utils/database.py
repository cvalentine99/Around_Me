...
def init_db() -> None:
    ...
    with get_db() as conn:
        ...
        # Add password_must_change column if upgrading from older schema
        try:
            conn.execute('SELECT password_must_change FROM users LIMIT 1')
        except Exception:
        ...
        if cursor.fetchone()[0] == 0:
            ...
            if not ADMIN_PASSWORD_EXPLICIT:
                logger.warning(
                    '\n'
                    '╔══════════════════════════════════════════════════════════════╗\n'
                    '║  GENERATED DEFAULT ADMIN PASSWORD (change on first login):  ║\n'
                    '║  %-58s  ║\n'
                    '╚══════════════════════════════════════════════════════════════╝',
                    ADMIN_PASSWORD
                )
            else:
                logger.info(f"Creating admin user: {ADMIN_USERNAME} (password set via env)")

            hashed_pw = generate_password_hash(ADMIN_PASSWORD)

            # Force password change unless operator explicitly set one via env
            must_change = 0 if ADMIN_PASSWORD_EXPLICIT else 1
