...
logger = logging.getLogger('valentine.updater')
...
def _get_github_repo() -> str:
    ...
    return getattr(config, 'GITHUB_REPO', 'smittix/valentine-rf')
...
def _fetch_github_release() -> dict[str, Any] | None:
    ...
    try:
        req = Request(url, headers={
            'User-Agent': 'Valentine RF-SIGINT',
    ...
    try:
        ...
        if stash_changes and git_status['has_changes']:
            stash_result = subprocess.run(
                ['git', 'stash', 'push', '-m', 'VALENTINE RF auto-stash before update'],
                capture_output=True,
                text=True,
                timeout=30,
                cwd=repo_root
