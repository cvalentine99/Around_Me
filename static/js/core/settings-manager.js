...
const Settings = {
    ...
    _loadFromLocalStorage() {
        const stored = localStorage.getItem('valentine_settings');
        if (stored) {
            try {
    ...
    async _save(key, value) {
        ...
        localStorage.setItem('valentine_settings', JSON.stringify(this._cache));
    ...
    _showReloadPrompt() {
        ...
        if (!prompt) {
            ...
            prompt.style.cssText = `
                ...
                border: 1px solid var(--accent-cyan, #a78bfa);
                border-radius: 8px;
                padding: 12px 16px;
                display: flex;
                align-items: center;
            ...
            prompt.innerHTML = `
                ...
                <button onclick="location.reload()" style="
                    background: var(--accent-cyan, #a78bfa);
                    border: none;
                    color: #000;
                    padding: 6px 12px;
                    border-radius: 4px;
...
async function toggleUpdateCheck(enabled) {
    ...
    localStorage.setItem('valentine_update_check_enabled', enabled ? 'true' : 'false');
