...
function plotReceiversOnMap(receivers) {
    ...
    receivers.forEach((rx, idx) => {
        ...
        const marker = L.circleMarker([rx.lat, rx.lon], {
            radius: 6,
            fillColor: rx.available ? '#a78bfa' : '#666',
            color: rx.available ? '#a78bfa' : '#666',
            weight: 1,
            opacity: 0.8,
            fillOpacity: 0.6,
        });
        ...
        marker.bindPopup(`
            <div style="font-size: 12px; min-width: 200px;">
                ...
                <button onclick="selectReceiver(${idx})" style="margin-top: 6px; padding: 4px 12px; background: #a78bfa; color: #000; border: none; border-radius: 3px; cursor: pointer; font-weight: bold;">Listen</button>
            </div>
        `);
