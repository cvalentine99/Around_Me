...
def start_wifi_scan():
    ...
    with app_module.wifi_lock:
        ...
        csv_path = '/tmp/valentine_wifi'

        for f in [f'/tmp/valentine_wifi-01.csv', f'/tmp/valentine_wifi-01.cap']:
            try:
                os.remove(f)
            except OSError:
                pass
...
def capture_handshake():
    ...
    with app_module.wifi_lock:
        ...
        capture_path = f'/tmp/valentine_handshake_{target_bssid.replace(":", "")}'
...
def capture_pmkid():
    ...
    with pmkid_lock:
        ...
        capture_path = f'/tmp/valentine_pmkid_{target_bssid.replace(":", "")}.pcapng'
