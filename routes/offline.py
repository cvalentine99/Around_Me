...
def check_asset():
    ...
    # Resolve with parent-directory containment check
    app_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    vendor_root = os.path.join(app_root, 'static', 'vendor')
    relative_name = path.removeprefix('/static/vendor/')
    try:
        full_path = resolve_safe(relative_name, vendor_root)
    except ValueError:
        return jsonify({'status': 'error', 'message': 'Invalid path'}), 400
